...
long double cosX(long double x, int pres) // calcula o cosseno de X
{
...
void menu() // menu primario do sistema
{
    ...
    if (aux == 1) // expressão em radianos
    {
        ...
        system("CLS");                      // limpa tela para o usuario
        if (Ajustar_90(x * 180) == 1)       // verifica se é 90 ou um valor de 90  + x*360
            printf("COS %Lf PI: %d", x, 0); // printa o valor para o usuario
        else
        {

            long double aux2 = x;
            x *= pi;                              // converte o valor em NUM
            long double aux = cos(x, pre);       // AUX recebe o cos aproximado de x(erro ao tentar fazer print direto)
    ...
    if (aux == 2) // expressão em graus
    {
        ...
        else
        {
            ...
            long double aux = cos(x, pre); // recebe resultado da funcao
